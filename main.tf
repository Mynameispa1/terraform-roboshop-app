...
# 2. In the process of autoscalling. 
    ...
    # g)Creating the autoscaling
    # h)AWS ALB rule terraform
...
resource "aws_ec2_instance_state" "component" {
  instance_id = module.component.id
  state       = "stopped"
  depends_on = [ null_resource.component ] #we need to mention depends_on becuase it will be stopped during the provisioning itself
}
...
resource "null_resource" "catalogue_delete" {
  ...
  provisioner "local-exec" {
    # Bootstrap script called with private_ip of each node in the cluster
    command = "aws ec2 terminate-instances --instance-ids ${module.component.id}" #check in google for the command
  }

   depends_on = [ aws_ami_from_instance.component ]
  # depends_on = [ aws_ami_from_instance.component, null_resource.component, aws_ec2_instance_state.component ]
}
...
# wa are cretaing a rule in component to forward request to perticular target group. In this case it is component
